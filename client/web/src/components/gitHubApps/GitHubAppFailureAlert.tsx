...
export const GitHubAppFailureAlert: React.FunctionComponent<React.PropsWithChildren<{ error: string }>> = ({
    ...
    <DismissibleAlert className="mb-3" variant="danger" error={error.toString()} className="mb-3" variant="danger">
        <div>
            <H4>Your GitHub App did not finish connecting to Sourcegraph correctly.</H4>
            <Text className="m-0">The following error occurred during setup: {error.toString()}</Text>
